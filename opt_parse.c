...
OPTS *popt;
{
	...
	case 'S':		/* step an instance */
	    weprintf("option ignored: instance stepping not yet implemented\n");
	    return(ERR);
	    break;
	case 'W':		/* width */
	    if(sscanf(optstring+2, "%lf", &optval) != 1) {
...
x ADD Llayer [:Wwidth] xy1 xy2 [xy3 ...]  EOC
  ADD Nlayer [:Mmirror] [:Rrot] [:Yyxratio] [:Zslant] [:Fsize] "string" xy EOC
  ADD Olayer [:Wwidth] [:Rres] xy1 xy2 xy3 EOC
  ADD Player [:Wwidth] xy1 xy2 xy3 [xy4 ...] EOC
